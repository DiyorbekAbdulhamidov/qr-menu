...
export default function AdminDashboard() {
  ...
  // Kategoriya va Tahrirlash uchun statelar
  ...
  return (
    <div className={cn(
      "min-h-screen font-sans selection:bg-[#D4AF37] selection:text-black relative pb-20",
      ...
      <div className="relative z-10 mx-auto max-w-[1400px] px-6 pt-20">
        ...
        <header className={cn(
          "mb-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 rounded-[2rem] p-8 backdrop-blur-2xl border transition-all duration-500",
          ...
          <div className="flex items-center gap-6">
            <div className="relative flex h-16 w-16 items-center justify-center rounded-[1.2rem] bg-gradient-to-br from-[#D4AF37] to-[#8A6D1C] shadow-[0_0_30px_rgba(212,175,55,0.3)]">
            ...
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-light tracking-tight" style={{ fontFamily: "var(--font-playfair)" }}>
                {restaurantName}
              </h1>
            </div>
          </div>
          ...
          <div className="flex w-full md:w-auto items-center gap-4">
            ...
            <button onClick={openAddModal} className="group relative flex h-12 items-center gap-3 overflow-hidden rounded-xl bg-gradient-to-r from-[#D4AF37] to-[#B38F24] px-8 font-bold text-black shadow-[0_10px_30px_rgba(212,175,55,0.3)] transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8">
              ...
              <nav className="no-scrollbar flex snap-x items-center gap-2 overflow-x-auto">
                {categories.map((cat) => (
                  <button
                    ...
                    className={cn(
                      "snap-center flex-shrink-0 whitespace-nowrap rounded-[1.5rem] px-6 py-3 text-[12px] font-bold uppercase tracking-wider transition-all duration-300",
                      activeCategory === cat
            ...
            {filteredItems.length === 0 ? (
              <div className={cn("flex flex-col items-center justify-center rounded-[2.5rem] border py-32 transition-colors", isDark ? "border-white/5 bg-white/[0.01]" : "border-black/5 bg-black/[0.01]")}>
                ...
                <h3 className="text-2xl font-light mb-2">Bu bo'lim hozircha bo'sh</h3>
                <p className="text-xs uppercase tracking-widest text-zinc-500">Tepadan "Yangi Taom" orqali to'ldiring</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {filteredItems.map((item) => (
                  <div key={item.id} className={cn(
                    ...
                    isDark ? "bg-[#0A0A0A] border-white/5 hover:border-[#D4AF37]/30" : "bg-white border-black/5 hover:border-[#D4AF37]/30",
                    !item.isAvailable && "opacity-60 grayscale-[30%]"
                  )}>

                    <div className="flex items-stretch gap-4 p-4">
                      {/* Premium Image Thumbnail */}
                      <div className="relative h-28 w-28 flex-shrink-0 overflow-hidden rounded-[1.2rem] bg-[#111]">
                        {item.imageUrl ? (
                          <img src={item.imageUrl} alt={item.name} className="h-full w-full object-cover object-center transition-transform duration-[2000ms] group-hover:scale-110" />
                        ) : (
                      ...
                      <div className="flex flex-1 flex-col py-1">
                        <h3 className="mb-1 line-clamp-2 text-lg font-bold leading-tight tracking-tight" style={{ fontFamily: "var(--font-playfair)" }}>{item.name}</h3>
                        <p className={cn("mb-3 line-clamp-2 text-[11px] font-light leading-relaxed", isDark ? "text-zinc-400" : "text-zinc-500")}>
                          {item.description || "Taom haqida ma'lumot kiritilmagan."}
                        </p>
                        <div className="mt-auto flex items-center justify-between">
                          <span className={cn("text-lg font-bold tracking-tight", isDark ? "text-[#D4AF37]" : "text-[#B38F24]")}>
                            {Number(item.price).toLocaleString()} <span className="text-[9px] uppercase tracking-widest opacity-70">UZS</span>
                          </span>
                        </div>
                      </div>
                    </div>

                    {/* Jiddiy va Elegant Control Bar */}
                    <div className={cn(
                      "flex items-center justify-between border-t px-2 py-2",
                      isDark ? "border-white/5 bg-[#111]/50" : "border-black/5 bg-zinc-50/50"
                    )}>
                      <button onClick={() => openEditModal(item)} className={cn("flex flex-1 items-center justify-center gap-2 rounded-xl py-2 text-[10px] font-bold uppercase tracking-widest transition-colors", isDark ? "text-blue-400 hover:bg-blue-500/10" : "text-blue-600 hover:bg-blue-500/10")}>
                        <Pencil size={14} strokeWidth={2} /> Tahrirlash
                      </button>
                      <div className={cn("h-4 w-px", isDark ? "bg-white/10" : "bg-black/10")} />
                      <button onClick={() => toggleAvailability(item.id, item.isAvailable)} className={cn("flex flex-1 items-center justify-center gap-2 rounded-xl py-2 text-[10px] font-bold uppercase tracking-widest transition-colors", item.isAvailable ? "text-amber-500 hover:bg-amber-500/10" : "text-emerald-500 hover:bg-emerald-500/10")}>
                        {item.isAvailable ? <><EyeOff size={14} strokeWidth={2} /> Yashirish</> : <><Eye size={14} strokeWidth={2} /> Ko'rsatish</>}
                      </button>
                      <div className={cn("h-4 w-px", isDark ? "bg-white/10" : "bg-black/10")} />
                      <button onClick={() => deleteItem(item.id)} className={cn("flex flex-1 items-center justify-center gap-2 rounded-xl py-2 text-[10px] font-bold uppercase tracking-widest transition-colors", isDark ? "text-red-400 hover:bg-red-500/10" : "text-red-600 hover:bg-red-500/10")}>
                        <Trash2 size={14} strokeWidth={2} /> O'chirish
                      </button>
                    </div>
          ...
          {/* RIGHT: VIP QR CARD (Unchanged Premium) */}
          <div className="lg:col-span-4">
            <div className="sticky top-10">
              <div className={cn(
                "relative overflow-hidden rounded-[2.5rem] border p-8 shadow-2xl transition-all duration-700",
                ...
                <div className="relative z-10 flex flex-col items-center">
                  ...
                  <h3 className="mb-2 text-2xl font-light tracking-tight">Eksklyuziv QR Code</h3>
                  ...
                  <a href={restaurantSlug ? `/menu/${restaurantSlug}` : "#"} target="_blank" rel="noopener noreferrer" className="mt-6 flex items-center justify-center gap-2 text-[11px] font-bold uppercase tracking-widest text-zinc-500 transition-colors hover:text-[#D4AF37]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-2xl transition-all duration-500">
          <div className={cn(
            ...
            <div className="flex items-center justify-between border-b border-white/5 px-10 py-8">
              <div>
                <h2 className="text-2xl font-light tracking-tight" style={{ fontFamily: "var(--font-playfair)" }}>
                  {editingId ? "Taomni Tahrirlash" : "Yangi Taom"}
                </h2>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#D4AF37] mt-2">Kolleksiya sozlamalari</p>
              </div>
            ...
            <div className="max-h-[75vh] overflow-y-auto px-10 py-8 no-scrollbar">
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div>
                  <label className="group relative flex h-[240px] w-full cursor-pointer flex-col items-center justify-center overflow-hidden rounded-[2rem] border border-white/10 bg-[#111] transition-all hover:border-[#D4AF37]/50">
                    {imagePreview ? (
                      <>
                        <img src={imagePreview} alt="Preview" className="h-full w-full object-cover object-center transition-transform duration-700 group-hover:scale-105 opacity-80" />
                        <div className="absolute inset-0 flex items-center justify-center bg-black/50 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                          <span className="rounded-full bg-white px-6 py-2 text-[10px] font-bold uppercase tracking-widest text-black shadow-2xl">Boshqa rasm yuklash</span>
                        </div>
                      </>
                    ) : (
                      <div className="flex flex-col items-center justify-center text-center">
                        <div className="mb-4 rounded-full border border-white/5 bg-white/5 p-5 shadow-2xl transition-transform duration-500 group-hover:scale-110 group-hover:border-[#D4AF37]/30">
                          <ImagePlus size={32} className="text-zinc-400 group-hover:text-[#D4AF37]" strokeWidth={1.5} />
                        </div>
                        <span className="text-sm font-light text-zinc-300">Suratni yuklang (Ixtiyoriy)</span>
                        <span className="mt-2 text-[10px] font-bold uppercase tracking-widest text-zinc-600">Avtomatik siqiladi</span>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2 md:col-span-2">
                    <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 ml-2">Taom Nomi <span className="text-red-500">*</span></label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 ml-2">Kategoriya <span className="text-red-500">*</span></label>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 ml-2">Narxi (UZS) <span className="text-red-500">*</span></label>
                ...
                <div className="flex gap-4 pt-4 border-t border-white/5">
                  <Button type="button" onClick={closeModal} className="h-16 flex-1 rounded-2xl border border-white/10 bg-transparent text-[11px] font-bold uppercase tracking-widest transition-colors hover:bg-white/5 text-zinc-400">
                    Bekor qilish
                  </Button>
                  <Button type="submit" isLoading={isSubmitting} className="h-16 flex-[2] rounded-2xl bg-gradient-to-r from-[#D4AF37] to-[#B38F24] text-[11px] font-bold uppercase tracking-widest text-black shadow-[0_10px_30px_rgba(212,175,55,0.3)] transition-transform hover:scale-[1.02] active:scale-[0.98]">
