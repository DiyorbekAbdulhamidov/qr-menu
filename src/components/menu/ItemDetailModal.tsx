...
import { MenuItem } from "@/lib/types"; // Yoki o'zingizdagi to'g'ri path
import { cn } from "@/lib/utils";
import Image from "next/image";
import { X, ChefHat, Plus, Minus, Info, ClipboardList } from "lucide-react";
...
export function ItemDetailModal({ item, onClose, onAdd, isDark, categories }: DetailProps) {
  ...
  // XATONI OLDINI OLUVCHI XAVFSIZ TEKSHIRUV:
  const parentCat = item ? categories.find(c => String(c.name || "").trim().toLowerCase() === String(item.category || "").trim().toLowerCase()) : null;

  let isAvailable = item?.isAvailable !== false;
  let timeText = "";

  if (parentCat && parentCat.startTime && parentCat.endTime) {
    const now = new Date();
    const current = now.getHours() * 60 + now.getMinutes();
    const [sh, sm] = parentCat.startTime.split(":").map(Number);
    const [eh, em] = parentCat.endTime.split(":").map(Number);
    const startMins = sh * 60 + sm;
    const endMins = eh * 60 + em;

    const isTimeAvailable = startMins <= endMins
      ? (current >= startMins && current <= endMins)
      : (current >= startMins || current <= endMins);

    isAvailable = isAvailable && isTimeAvailable;
    timeText = `${parentCat.startTime} - ${parentCat.endTime}`;
  }
    ...
    <AnimatePresence>
      {item && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center" role="dialog">
          ...
          {/* Shaffof Orqa Fon (Backdrop Blur) */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          {/* Asosiy Modal Oyna (Spring Animatsiya) */}
          <motion.div
            initial={{ y: "100%", opacity: 0.5 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: "100%", opacity: 0.5 }}
            transition={{ type: "spring", stiffness: 350, damping: 30 }}
            className={cn("relative w-full h-[90vh] sm:h-auto sm:max-h-[90vh] sm:max-w-[480px] flex flex-col rounded-t-[32px] sm:rounded-[32px] overflow-hidden shadow-2xl border",
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col">
              {item.imageUrl ? (
                <div className="relative w-full aspect-square sm:aspect-[4/3] shrink-0 bg-zinc-200 dark:bg-[#0A0A0A]">
                ...
                {!isAvailable && (
                  <div className="flex items-center gap-3 bg-red-500/10 text-[#C8102E] px-4 py-3 rounded-2xl mb-6 border border-red-500/20">
                    ...
                    <span className="text-[12px] font-bold uppercase tracking-wider">
                      {timeText ? `Ish vaqti: ${timeText}` : "Ayni vaqtda mavjud emas"}
                    </span>
                  </div>
                )}
            ...
            {/* Pastki qotirilgan qism (Bodrum Red Button) */}
            {isAvailable && (
              <div className={cn("shrink-0 p-6 sm:px-8 sm:py-6 border-t", isDark ? "border-white/5" : "border-black/5")}>
                ...
                <button onClick={() => { onAdd(qty); onClose(); }} className="w-full py-4 rounded-[20px] bg-[#C8102E] text-white text-[15px] font-bold tracking-wide active:scale-[0.98] transition-transform flex justify-center items-center gap-2 shadow-[0_10px_20px_rgba(200,16,46,0.2)]">
                  <ClipboardList size={20} strokeWidth={2.5} /> Ro'yxatga qo'shish
                </button>
              </div>
            )}
          </motion.div>
